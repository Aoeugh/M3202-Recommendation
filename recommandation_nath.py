...
# Calcul de la moyenne pondérée des notes des utilisateurs choisis (faire varier
# le filtre des utilisateurs) (voir pour peut être changer les poids de chaque note);
...
# TEST SUR LES ITEMS AU LIEU DES USERS
...
# def calculSimilaritePearson(tableau,colonne): #On calcul la similarité
#     for i in range (0,tableau[1].shape[0]): #On parcourt tous les items
#         item1=[]
#         item2=[]
#         for utilisateurs in range (tableau.shape[0]): #On parcourt tous les utilisateurs
#             if (tableau[utilisateurs][i]!=-1 and tableau[utilisateurs][colonne]!=-1): 
#             #On vérifie que l'item est noté par l'utilisateur avec qui on veut comparer et 
#             #par l'utilisateur i
#                 item1.append(tableau[utilisateurs][colonne]) #On prend en compte la note
#                 item2.append(tableau[utilisateurs][i])     #On prend en compte la note
...
#         moyenneCoefItem1 = sommeMoy1 / len(item1) #moyenne des coefs item1
#         moyenneCoefItem2 = sommeMoy2 / len(item2) #moyenne des coefs item2
        
#         somme=0
#         somme1=0
#         somme2=0
        
#         for k,j in zip(item1, item2):
#             somme1 += (k - moyenneCoefItem1) * (k - moyenneCoefItem1)
#             somme2 += (j - moyenneCoefItem2) * (j - moyenneCoefItem2)
#             somme  += (k - moyenneCoefItem1) * (j - moyenneCoefItem2)
        
#         pearson = somme / ((sqrt(somme1)) * (sqrt(somme2)))
#         #On ajoute à la liste des similarite, celle entre "i" et "colonne"
#         similaritePearson.append(pearson)
    
#     return similaritePearson 
#     #C'est une liste avec toutes les similarite entre "colonne" et tous les items


# def calculSimilariteCosinus(tableau,colonne): 
#     for i in range (0,tableau[0].shape[0]): #Nombre de comparaisons avec l'utilisateur "ligne"
#         item1=[]
#         item2=[]
#         for utilisateurs in range (tableau.shape[0]): #On parcourt tous les utilisateurs
#             if (tableau[utilisateurs][colonne]!=-1 and tableau[utilisateurs][i]!=-1):
#                 item1.append(tableau[utilisateurs][colonne])
#                 item2.append(tableau[utilisateurs][i])
...
# # Méthode de la moyenne pour compléter les notes d'un utilisateur.
# def methodeMoyenne (tableau,similarite,palierSimilaires,colonne) :
#     for j in range (0,tableau.shape[0]): #On parcourt tous les utilisateurs de l'item
#         listePourMoyenneNotes=[]
#         moyenneNotes=0
#         if tableau[j][colonne] == -1: #Si l'utilisateur de l'item n'est pas rentré
#             for k in range (0,tableau[j].shape[0]): #On parcourt tous les items
#                 if tableau[j,k] != -1: #Si l'item k est noté par l'utilisateur j
#                     if similarite[k] > palierSimilaires:
#                         #Si la similarite entre j et notre item est suffisante
#                         #On ajoute la note de l'utilisateur k a une liste.
#                         listePourMoyenneNotes.append(tableau[j][k])
...
#             #On remplit un tableau de 0 avec les notes obtenues (en valeur exacte)
#             tableauNotesExactes[j,colonne]=float(note)
#             #On ajoute un arrondi de la valeur exacte à un autre tableau
#             tableau[j,colonne]=round(note)
...
# # Calcul des erreurs
# def calculBiais(tableau,tableauReponse):
#     erreur=[]
#     for i in range (tableau[1].shape[0]): #On compare tous les items
#         for j in range (tableau.shape[0]): #On compare tous les utilisateurs
#             if (tableauReponse[j,i]!=0.0):
#                 erreur.append(tableau[j,i]-tableauReponse[j,i])
...
# def calculErreurMoyenne (tableau, tableauReponse):
#     erreur=[]
#     for i in range (tableau[1].shape[0]): #On ne compare tous les items
#         for j in range (tableau.shape[0]): #On compare tous les utilisateurs
#             if (tableauReponse[j,i]!=0.0):
#                 erreur.append(abs(tableau[j,i]-tableauReponse[j,i]))    
...
# def scenarioPearson(tableau, tableauReponse,palierSimilaires):
#     for i in range (0,tableau[1].shape[0]):
#         #On calcul les similarite entre la colonne "i" et toutes les colonnes du tableau
#         similarite=calculSimilaritePearson(tableau,i) 
#         """On va calculer les notes à rentrer dans le tableau.
#         - tableau étant le tableau non complété, 
#         - similarite est la liste de similarite entre l'item "i" et les autres
#         - palierSimilaire est le pallier que l'on choisit pour prendre en compte un item
#             a partir de sa similarite avec l'item "i"
#         - i est l'item
#         """
#         tableau, tableauNotesExactes=methodeMoyenne(tableau, similarite,palierSimilaires,i)
...
# def scenarioCosinus(tableau, tableauReponse,palierSimilaires):
#     for i in range (0,tableau[1].shape[0]):
#         #On calcul les similarite entre la colonne "i" et toutes les autres
#         similarite=calculSimilariteCosinus(tableau,i) 
#         """On va calculer les notes à rentrer dans le tableau.
#         - tableau étant le tableau non complété, 
#         - similarite est la liste de similarite entre l'item "i" et tous les autres
#         - palierSimilaire est le pallier que l'on choisit pour prendre en compte un item
#             a partir de sa similarite avec l'item "i"
#         - i est l'item
#         """
...
#     biais=calculBiais(tableau, tableauReponse)
#     erreur=calculErreurMoyenne(tableau, tableauReponse)
#     print("Le biais est de : ",biais)
#     print("L'erreur moyenne est de : ",erreur)
